...
for iSystem = 1:numel(systemDefinitions)
    ...
    residualNum = computeStaticResiduum(systemNum, parameters, q, u);

    qSym = casadi.MX.sym('q', systemNum.nDoF, 1);
    uSym = casadi.MX.sym('u', systemNum.nInputs, 1);
    residualSym = computeStaticResiduum_casadi( ...
        systemSym, parameters, qSym, uSym);
...
for iSystem = 1:numel(systemDefinitions)
    ...
    residualSym = computeDELResiduum_casadi( ...
...
for iSystem = 1:numel(systemDefinitions)
    ...
    firstStepResidualNum = computeDELResiduumFirstStep( ...
        systemNum, h, parameters, q_0, q_1, u_0, a);
    firstStepResidualSym = full(computeDELResiduumFirstStep_casadi( ...
    ...
    lastStepResidualNum = computeDELResiduumFinalStep( ...
        systemNum, h, parameters, q_0, q_1, u_0, qDotEnd, a);
    lastStepResidualSym = full(computeDELResiduumFinalStep_casadi( ...
...
for iSystem = 1:numel(systemDefinitions)
    ...
    rhsNum = computeFirstOrderSystemRHS_MInv( ...
        0, x, systemNum, parameters);
    rhsSym = full(computeFirstOrderSystemRHS_MInv_casadi( ...
