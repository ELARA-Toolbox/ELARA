function [res_N, eta_N0] = DELResidualFinalStep(MBSys, h, simPars, q_N0, q_N, u_N, qDotEnd, a)
    ...
    p_N = MBSys.computeMassMatrixFast(J_N) * qDotEnd;
    ...
    f_gen_N = ...
        ...
        - a * MBSys.computeInputMatrixFast(g_rel_N) * u_N;
    
    % DEL Eqs. / Legendre transform at last step
    res_N = -h*f_gen_N - p_N;
    for iFrm = 1:MBSys.nFrames
        res_N = res_N + J_N(:,:,iFrm).' * (...
            + elara.SE3.dcayInv(-eta_N0(:,iFrm)*h).' * MBSys.frames.MGen(:,:,iFrm) * eta_N0(:,iFrm) ...
            - h*f_frame_N_b(:, iFrm) ...
            );
    end
end
