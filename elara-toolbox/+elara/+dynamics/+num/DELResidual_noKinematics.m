function DEL_res_k = DELResidual_noKinematics( MBSys, simPars, ...
    ...
    % frames
    f_frame_k_b = elara.dynamics.num.bodyFixedFrameForces(g_k, f_frame_k_s_ext, MBSys, simPars);
    ...
    for iFrm = 1:MBSys.nFrames
        DEL_res_k = DEL_res_k ...
            + J_k(:,:,iFrm).' *( ...
            1/h*elara.SE3.dcayInv(eta_k(:,iFrm)*h).' * MBSys.frames.MGen(:,:,iFrm) * eta_k(:,iFrm) ...
            - 1/h*elara.SE3.dcayInv(-eta_k0(:,iFrm)*h).' * MBSys.frames.MGen(:,:,iFrm) * eta_k0(:,iFrm) ...
            + f_frame_k_b(:, iFrm) ...
