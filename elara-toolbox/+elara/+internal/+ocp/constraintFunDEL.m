function [c, lb_c, ub_c, g, c_DEL, c_WS] = constraintFunDEL(OCP, q, u, opts)
    ...
    for k = 2:nSteps
        ...
        lb_c{k,1} = zeros(MBSys.nDoF, 1);
        ub_c{k,1} = zeros(MBSys.nDoF, 1);
        g{k} = g_k;
    ...
    g{end} = MBSys.computeFwdKin(q{nSteps+1});
    ...
    if ~isempty(OCP.qDotF)
        g_rel_N  = MBSys.computeJointTransformations(q{nSteps});
        ...
        lb_c = [lb_c; zeros(MBSys.nInputs,1)];
        ub_c = [ub_c; zeros(MBSys.nInputs,1)];
    end
