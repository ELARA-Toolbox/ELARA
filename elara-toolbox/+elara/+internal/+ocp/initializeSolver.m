function [NLPSolver, constrDef] = initializeSolver(OCP, opts)
    ...
    if ~OCP.systemSym.indexTCPFrame
        warning("No TCP frame defined in the ELARA System object. Using last system frame as the TCP frame.")
        indexTCPFrame = OCP.systemSym.nFrames;
    else
        indexTCPFrame = OCP.systemSym.indexTCPFrame;
    end
    ...
    if OCP.discretization.type == "varint"
        if OCP.useSplineInputs
            ...
            [q_C, z_C] = elara.internal.ocp.unpackSplineDecisionVariables( ...
                X_nlp, OCP.nSteps, nDoF, ...
                nInputs, OCP.nInputSplinePoints, "cell", true);
        else
            nVarsStep  = nDoF + nInputs;
            X_nlp = casadi.MX.sym('x', nVarsStep*(nSteps+1), 1);
            [q_C, u_C] = elara.internal.ocp.unpackNodeDecisionVariables( ...
                X_nlp, nSteps, nDoF, ...
                nInputs, "cell", true);
        end
        ...
        if OCP.useSplineInputs
            ...
            x_C = elara.internal.ocp.unpackSplineDecisionVariables( ...
                X_nlp, OCP.nSteps, 2*nDoF, ...
            ...
            X_nlp = casadi.MX.sym('x', nVarsStep*(nSteps+1), 1);
            x_C = elara.internal.ocp.unpackNodeDecisionVariables( ...
                X_nlp, nSteps, 2*nDoF, nInputs, "cell", true, "isODEDiscr", false);
            [q_C, u_C, ~] = elara.internal.ocp.unpackNodeDecisionVariables( ...
                X_nlp, nSteps, nDoF, nInputs, "cell", true, "isODEDiscr", true);
        end
    end
    ...
    g_TCP_F = g_F(indexTCPFrame) * elara.SE3.Element(OCP.systemSym.g_B_TCP(1:3,1:3), OCP.systemSym.g_B_TCP(1:3,4));
    ...
    if OCP.runningCostActive(5)
        for iStep = 1:OCP.nSteps + 1
            g_TCP_k = g{iStep}(indexTCPFrame) * elara.SE3.Element(OCP.systemSym.g_B_TCP(1:3,1:3), OCP.systemSym.g_B_TCP(1:3,4));
