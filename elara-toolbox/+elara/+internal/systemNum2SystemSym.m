function systemSym = systemNum2SystemSym(systemNum)
    ...
    systemSym = elara.SystemSym;
    for fn = fieldnames(systemNum)'
        try
            systemSym.(fn{1}) = systemNum.(fn{1});
        catch
            % Ignore properties that are not shared or cannot be converted.
        end
    end
    for fn = fieldnames(systemNum.frames)'
        try
            systemSym.frames.(fn{1}) = systemNum.frames.(fn{1});
        catch
            % Ignore properties that are not shared or cannot be converted.
        end
    end
